...
export class AuthService {
  ...
  registerUser(user): any {
    ...
    let observableReq = this.http
      .post('http://localhost:8080/users/register', reqBody, options)
  ...
  authenticateUser(user): any {
    ...
    // POST
    let observableReq = this.http
      .post('http://localhost:8080/users/authenticate', reqBody, options)
      .map(this.extractData);
  ...
  getProfile(): any {
    ...
    // POST
    let observableReq = this.http.get('http://localhost:8080/users/profile', options).map(this.extractData);
